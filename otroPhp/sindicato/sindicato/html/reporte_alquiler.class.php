...
class reporte_chofer
{
    ...
    $fechaInicio = $_GET[fi];
		...
		$hojas = $query->getRows("*","hoja_control","where id_movil = ".$pertenece['id_movil']." and fecha_a_usar >= '".$fechaInicio."' and fecha_a_usar <= '".$fechaFin."'");
                $hojasUsadas = $query->getRows("*","hoja_control","where id_movil = ".$pertenece['id_movil']." and hoja_usada = 'Recibido' and fecha_a_usar >= '".$fechaInicio."' and fecha_a_usar <= '".$fechaFin."'");
$numHojas = count($hojas);
        ...
        */
        ...
        if($numsocio > 0) {
            ...
            foreach ($resultmovil1 as $key=>$value) {
				...
				$hojas = $query->getRows("*","hoja_control h,movil m,linea l","where h.id_movil=m.id_movil and m.id_linea=l.id_linea and l.id_linea='2' and id_chofer = ".$value['id_chofer']."  and fecha_de_compra >= '".$fechaInicio."' and fecha_de_compra <= '".$fechaFin."'");
				$numHojas = count($hojas);
    ...
    function FiltrarFechas() //list for default the all items
	{
		...
		$template->SetTemplate('html/reporte_chofer.html'); //sets the template for this function
