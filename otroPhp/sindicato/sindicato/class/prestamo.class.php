...
class prestamo
{
	...
	function registrarPrestamo(){
        ...
        $template->SetParameter('licencia',"<input name=\"licencia\" type=\"text\" id=\"licencia\" onblur=\"if(document.mframe.licencia.value > 0) {ajax('nombreSocio','prestamo.php?accion=buscarSocio&licencia=' + document.mframe.licencia.value, '');}\" size=\"8\">");
		
		$template->SetParameter('licencia2',"<input name=\"licencia2\" type=\"text\" id=\"licencia2\" onblur=\"if(document.mframe.licencia2.value > 0) {ajax('nombreChofer','prestamo.php?accion=buscarChofer&licencia=' + document.mframe.licencia2.value, '');}\" size=\"8\">");
    ...
    function modificarPrestamo(){
		...
		$template = new template;
		$template->SetTemplate('html/form_prestamo.html');
        $template->SetParameter('licencia',"<input name=\"licencia\" type=\"text\" id=\"licencia\" onblur=\"if(document.mframe.licencia.value > 0) {ajax('nombreSocio','prestamo.php?accion=buscarSocio&licencia=' + document.mframe.licencia.value, '');}\" size=\"8\" value=".$socio['num_licencia'].">");
		
		$template->SetParameter('licencia2',"<input name=\"licencia2\" type=\"text\" id=\"licencia2\" onblur=\"if(document.mframe.licencia2.value > 0) {ajax('nombreChofer','prestamo.php?accion=buscarChofer&licencia=' + document.mframe.licencia2.value, '');}\" size=\"8\" value=".$chofer['num_chofer'].">");
    ...
    function deletePrestamo(){
		...
		if(count($deposito) != 0) {
		 $query->dbDelete("deposito","WHERE id_prestamo = ".$_GET['id']);
          ...
          echo "<script>window.location.href='prestamo.php'</script>";
	       }
	    }
		}
	...
	function calcularPlan()
	{
		...
		echo "<script>window.location.href='prestamo.php?accion=plan&id=".$_POST['id_prestamo']."'</script>";
	}
	...
	function guardarPago()
	{
		...
		//}
		echo "<script>window.location.href='prestamo.php?accion=pago&id=".$_POST['id_prestamo']."'</script>";
	}
	function deletePago(){
        $query = new query;
	...
	{
		$template = new template;
		$template->SetTemplate('html/lista_pagos.html'); //sets the template for this function
...
$numerarcpx=$numerarcpx+1;
	...
	function ingresarPago()//list of all payments and new ones
	{
        ...
        if($numdeposito > 0) {
            ...
            foreach ($resultdeposito as $key=>$value) {
                ...
                if(($x%2)==0)
				  ...
				  <td><a href="prestamo.php?accion=deleteP&id='.$value["id_deposito"].'" onClick="'.$confirm.'" title="Eliminar Deposito"><img src="images/delete.gif"></a></td>
		...
		$template->SetParameter('interes',$interes['interes']);
		$template->SetParameter('total',$totalPago);
        $buttonAdd = '<input type="button" value="GUARDAR" onclick="window.location.href=\'prestamo.php?accion=guardarPago&id='.$_GET['id'].'\'">';
	...
	{
		$template = new template;
		$template->SetTemplate('html/lista_prestamo.html'); //sets the template for this function
	...
	$interes = $query ->getRow("*","tipo_prestamo","where id_tipo_prestamo = ".$value['id_tipo_prestamo']);
			...
			$totalPago = $value['monto_prestamo'] + ($value['monto_prestamo']*$interes['interes']/100);
                ...
                $list .= '<tbody><tr '.$par.'>
				  ...
				  <td><a href="prestamo.php?accion=plan&id='.$value["id_prestamo"].'" title="Plan de pagos"><img src="images/plan.gif"></a></td>
				  <td><a href="prestamo.php?accion=pago&id='.$value["id_prestamo"].'" title="Pagar Prestamo"><img src="images/pago.gif"></a></td>
                  <td><a href="prestamo.php?accion=modificar&id='.$value["id_prestamo"].'" title="Modificar Prestamo"><img src="images/edit.gif"></a></td>
                  <td><a href="prestamo.php?accion=delete&id='.$value["id_prestamo"].'" onClick="'.$confirm.'" title="Eliminar prestamo"><img src="images/delete.gif"></a></td>
            ...
            $list.='</table>';
            //$list .= paging::navigation(count($resultprestamo),"prestamo.php",10);
        } else $list = '<div>No existen prestamos registrados</div>';
        $buttonAdd = '<input type="button" value="Nuevo" onclick="window.location.href=\'prestamo.php?accion=registro\'">';
        $template->SetParameter('add',$buttonAdd);
