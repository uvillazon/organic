...
class arqueo
{
	...
	function registrarPrestamo(){
        ...
        $template->SetParameter('licencia',"<input name=\"licencia\" type=\"text\" id=\"licencia\" onblur=\"if(document.mframe.licencia.value > 0) {ajax('nombreSocio','arqueo132.php?accion=buscarSocio&licencia=' + document.mframe.licencia.value, '');}\" size=\"8\">");
		
		$template->SetParameter('licencia2',"<input name=\"licencia2\" type=\"text\" id=\"licencia2\" onblur=\"if(document.mframe.licencia2.value > 0) {ajax('nombreChofer','arqueo132.php?accion=buscarChofer&licencia=' + document.mframe.licencia2.value, '');}\" size=\"8\">");
    ...
    function modificarPrestamo(){
        ...
        $template->SetParameter('licencia',"<input name=\"licencia\" type=\"text\" id=\"licencia\" onblur=\"if(document.mframe.licencia.value > 0) {ajax('nombreSocio','arqueo132.php?accion=buscarSocio&licencia=' + document.mframe.licencia.value, '');}\" size=\"8\" value=".$socio['num_licencia'].">");
		
		$template->SetParameter('licencia2',"<input name=\"licencia2\" type=\"text\" id=\"licencia2\" onblur=\"if(document.mframe.licencia2.value > 0) {ajax('nombreChofer','arqueo132.php?accion=buscarChofer&licencia=' + document.mframe.licencia2.value, '');}\" size=\"8\" value=".$chofer['num_chofer'].">");
    ...
    function deletePrestamo(){
		...
		if(count($deposito) != 0) {
		 $query->dbDelete("deposito","WHERE id_prestamo = ".$_GET['id']);
          ...
          echo "<script>window.location.href='arqueo132.php'</script>";
	       }
	    }
		}
   ...
   //$totalPago = $query->getRow("SUM(monto_ingreso_dolar) as total","ingreso","where fecha_ingreso='".$hoy."'");
    //  $ingresosus = $totalPago['total'];
	 $resultplan = $query->getRow("SUM(i.monto_pago_ingreso)AS totalbs","linea l, movil m, ingreso i,tipo_ingreso t","where i.id_tipo_ingreso=t.id_tipo_ingreso and l.id_linea=m.id_linea and m.id_movil=i.id_movil and l.id_linea='2' and  i.fecha_ingreso='".$hoy."'");
	...
	$resultplan1 = $query->getRow("SUM(i.monto_ingreso_dolar)AS totalbs","linea l, movil m, ingreso i,tipo_ingreso t","where i.id_tipo_ingreso=t.id_tipo_ingreso and l.id_linea=m.id_linea and m.id_movil=i.id_movil and l.id_linea='2' and  i.fecha_ingreso='".$hoy."'");
	...
	$totalDiaBs = $query->getRow("SUM(i.monto_pago_ingreso) as totalDiabs","ingreso i,tipo_ingreso t","where i.id_tipo_ingreso=t.id_tipo_ingreso and t.linea='2' and i.fecha_ingreso='".$hoy."'");
	 ...
	 $resultplan2 = $query->getRow("SUM(monto_egreso) as total","egresos132","WHERE fecha='".$hoy."'");
	 $egresobs = $resultplan2['total'];
  	$resultplan3 = $query->getRow("SUM(monto_dolar) as total","egresos132","WHERE fecha='".$hoy."'");
  	 $egresosus = $resultplan3['total'];
	
	$totalPagoa = $query->getRow("SUM(h.aporte) as total","linea l, movil m, hoja_control h","where l.id_linea=m.id_linea and m.id_movil=h.id_movil and l.id_linea='2' and  h.fecha_de_compra='".$hoy."'");
      $ingresohojasaporte = $totalPagoa['total'];

	  $totalPagob = $query->getRow("SUM(h.adeporte) as total","linea l, movil m, hoja_control h","where l.id_linea=m.id_linea and m.id_movil=h.id_movil and l.id_linea='2' and  h.fecha_de_compra='".$hoy."'");
     $ingresohojasdeporte = $totalPagob['total'];
	 
	  $totalIngresosBS = $ingresobs + $ingresohojasaporte +$ingresohojasdeporte;
			$interes = $query ->getRow("MAX(idcaja) as ultimo","caja132");
 $idcajan = $interes['ultimo'];

	 $cajaanteriorbs = $query->getRow("cajanueva as total","caja132","where idcaja='".$idcajan."'");
	  $cajaanteriorbs = $cajaanteriorbs['total'];
     $cajaanteriorsus = $query->getRow("cajanuevasus as total","caja132","where idcaja='".$idcajan."'");
...
$totaldisponible= $totalIngresosBS - $egresobs + $cajaanteriorbs;
	...
	//	echo "<script>window.location.href='arqueo132.php?accion=plan&id=".$_POST['id_prestamo']."'</script>";
	}
	...
	function guardarPago()
	{
		...
		//}
		echo "<script>window.location.href='arqueo132.php?accion=pago&id=".$_POST['id_prestamo']."'</script>";
	}
	function deletePago(){
        $query = new query;
...
$totalEgresosSUS += $value["monto_dolar"];
 ...
 // $totalDiaBs = $query->getRow("SUM(i.monto_pago_ingreso) as totalDiabs","ingreso i,tipo_ingreso t","where i.id_tipo_ingreso=t.id_tipo_ingreso and t.linea='1' and i.arqueo='0'");
      //  $totalDiaDs = $query->getRow("SUM(i.monto_ingreso_dolar) as totalDiadl","ingreso i,tipo_ingreso t","where i.id_tipo_ingreso=t.id_tipo_ingreso and t.linea='1' and i.arqueo='0'");
        ...
        $buttonAdd = '<input type="button" value="Arquear" onclick="window.location.href=\'arqueo132.php?accion=calcularPlan&id='.$_GET['id'].'\'">';
...
$totalEgresosSUS += $value["monto_dolar"];
 ...
 // $totalDiaBs = $query->getRow("SUM(i.monto_pago_ingreso) as totalDiabs","ingreso i,tipo_ingreso t","where i.id_tipo_ingreso=t.id_tipo_ingreso and t.linea='1' and i.arqueo='0'");
		...
		function ingresarPago()//list of all payments and new ones
	{
        ...
        if($numdeposito > 0) {
            ...
            foreach ($resultdeposito as $key=>$value) {
                ...
                if(($x%2)==0)
				  ...
				  <td><a href="arqueo132.php?accion=deleteP&id='.$value["id_deposito"].'" onClick="'.$confirm.'" title="Eliminar Deposito"><img src="images/delete.gif"></a></td>
		...
		$template->SetParameter('total',$totalPago);
        $buttonAdd = '<input type="button" value="GUARDAR" onclick="window.location.href=\'arqueo132.php?accion=guardarPago&id='.$_GET['id'].'\'">';
     ...
     // $ingresosus = $totalPago['total'];
      ...
      $egresosus = $totalPago['total'];
		 ...
		 $cajaanteriorsus = $value["cajanuevasus"];
                ...
                <td>'.$saldobs.'</td>
                  <td>'.$saldosus.'</td>
				  <td><a href="arqueo132.php?accion=plan&id='.$value["id_caja"].'" title="Ver detalle movimiento"><img src="images/plan.gif"></a></td>
				  <td><a href="arqueo132.php?accion=planprint&id='.$value["id_caja"].'" 
            ...
            $list.='</table>';
            //$list .= paging::navigation(count($resultprestamo),"arqueo132.php",10);//lorgio
			// <td><a href="arqueo132.php?accion=delete&id='.$value["id_caja"].'" onClick="'.$confirm.'" title="Eliminar "><img src="images/delete.gif"></a></td>
				 
        } else $list = '<div>No existen montos registrados</div>';
        $buttonAdd = '<input type="button" value="Nuevo" onclick="window.location.href=\'arqueo132.php?accion=registro\'">';
        $template->SetParameter('add',$buttonAdd);
     ...
     $ingresohojasdeporte = $totalPagob['total'];
      ...
      $egresosus = $totalPago['total'];
		 ...
		 $cajaanteriorsus = $value["cajanuevasus"];
                ...
                <td>'.$saldobs.'</td>
                  <td>'.$saldosus.'</td>
		   <td><a href="arqueo132.php?accion=planprint&id='.$fechaFiltro.'" 
				 target="_blank" onClick="window.open(this.href, this.target); return false;"  title="Imprimir Informe">[Imprimir detalle]</a></td>
