...
class tipoingreso
{
	...
	function registrarTipoIngreso(){
		$template = new template;
		$template->SetTemplate('html/form_saldoingreso.html');
		$hoy=date("Y-m-d");
    ...
    function deleteTipoIngreso(){
        ...
        echo "<script>window.location.href='saldoingreso.php'</script>";
	}
    ...
    function saveTipoIngreso() //save the new Item
    {
		...
		$insert['saldo_tipo_dolar'] = $_POST[monto_tipo_ingresoa];
		$insert['fecha_registro_saldo'] = $_POST[fecha_registro_saldo];
		
		$insert['tipo_saldo'] = "ingreso";
	...
	{
		$template = new template;
		$template->SetTemplate('html/lista_saldoingreso.html'); //sets the template for this function
		
		//DataBase Conexion//
		$query = new query;
        $confirm="javascript:return confirm('Esta seguro de eliminar este saldo?');";
        //$resultingreso = $query->getRows("*","saldo");
  $resultingreso1 = $query->getRows("*","saldo","where tipo_saldo='ingreso'");
        ...
        if($numingreso > 0) {
            ...
            foreach ($resultingreso1 as $key=>$value) {
				...
				$list .= '<tbody><tr '.$par.'>
				  ...
				  <td>'.$value["saldo_tipo_dolar"].'</td>
				   <td>'.$value["fecha_registro_saldo"].'</td>
				 
                  <td><a href="saldoingreso.php?accion=modificar&id='.$value["id_saldo"].'" title="Modificar saldo "><img src="images/edit.gif"></a></td>
                  <td><a href="saldoingreso.php?accion=delete&id='.$value["id_saldo"].'" onClick="'.$confirm.'" title="Eliminar saldo"><img src="images/delete.gif"></a></td>
                  </tr></tbody>';
            }
            $list.='</table>';
           // $list .= paging::navigation(count($resultingreso),"saldoingreso.php",20);
        } else $list = '<div>No existen saldos registrados</div>';
        $buttonAdd = '<input type="button" value="Nuevo" onclick="window.location.href=\'saldoingreso.php?accion=registro\'">';
