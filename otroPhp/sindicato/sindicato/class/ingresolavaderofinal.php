...
class tipoingreso
{
	...
	function registrarTipoIngreso(){
		$template = new template;
		$template->SetTemplate('html/form_ingresolavaderofinal.html');
		$hoy=date("Y-m-d");
    ...
    function deleteTipoIngreso(){
        ...
        echo "<script>window.location.href='jingresolavadero.php'</script>";
	}
	...
	{
		$template = new template;
		$template->SetTemplate('html/lista_tipolavadero.html'); //sets the template for this function
        ...
        $resultingreso1 = $query->getRows("*","saldo","where tipo_saldo='lavadero'");
        ...
        if($numingreso > 0) {
            ...
            foreach ($resultingreso1 as $key=>$value) {
				...
				$list .= '<tbody><tr '.$par.'>
                  ...
                  <td><a href="jingresolavadero.php?accion=modificar&id='.$value["id_saldo"].'" title="Modificar saldo "><img src="images/edit.gif"></a></td>
                  <td><a href="jingresolavadero.php?accion=delete&id='.$value["id_saldo"].'" onClick="'.$confirm.'" title="Eliminar saldo"><img src="images/delete.gif"></a></td>
                  </tr></tbody>';
            }
            $list.='</table>';
      //      $list .= paging::navigation(count($resultingreso),"ingresolavadero.php",20);
        } else $list = '<div>No existen saldos registrados</div>';
        $buttonAdd = '<input type="button" value="Nuevo" onclick="window.location.href=\'jingresolavadero.php?accion=registro\'">';
