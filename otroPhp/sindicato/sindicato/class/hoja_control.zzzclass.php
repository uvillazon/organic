...
class hoja_control
{
	...
	function registrarHoja(){
        ...
        $check1 = "";
        $check2 = "";
		//$template->SetParameter('radio',"<input ".$check1." name=\"tipoSocio\" id=\"tipoSocio\" type=\"radio\" checked value=\"0\" onchange=\"Hide('showFechaFin');\"/><label>Normal</label> <input ".$check2." name=\"tipoSocio\" id=\"tipoSocio\" type=\"radio\" value=\"1\" onchange=\"Show('showFechaFin');\"/><label>Falta</label>");
		$template->SetParameter('radio',"<input ".$check1." name=\"tipoSocio\" id=\"tipoSocio\" type=\"radio\" checked value=\"0\" onchange=\"Hide('showFechaFin');\"/><label>Normal</label> <input ".$check2." name=\"tipoSocio\" id=\"tipoSocio\" type=\"radio\" value=\"1\" onchange=\"Show('showFechaFin');\"/><label>Falta</label>");
		...
		//$template->SetParameter('numeroMovil','');
		
		if($check1=="checked"){
			$template->SetParameter('numeroMovil',"<input name=\"movil\" type=\"text\" id=\"movil\" onblur=\"if(document.mframe.movil.value > 0){ajax('datosMovil','hoja_control.php?accion=buscarMovil&movil=' + document.mframe.movil.value, '');}\" size=\"3\">");
		}else{if($check2=="checked"){
			$template->SetParameter('numeroMovil',"<input name=\"movil\" type=\"text\" id=\"movil\" onblur=\"if(document.mframe.movil.value > 0){ajax('datosMovil','hoja_control.php?accion=buscarMovil2&movil=' + document.mframe.movil.value, '');}\" size=\"3\">");
		}}
	 ...
	 function buscarMovil(){
        ...
        $nombreSocio = $query->getRow("m.id_movil, nombre_socio, s.id_socio, apellido1_socio, apellido2_socio, placa_movilidad, estado_socio","socio s, movil m, pertenece p","where (m.num_movil like '132_".$_GET['movil']."' or m.num_movil like '131_".$_GET['movil']."') and m.id_movil = p.id_movil and p.id_socio = s.id_socio");
		$hoy = date('Y-m-d');
 $valoresPrimera = explode("-", $hoy);
...
$dia3 = $maxdays2; 
	...
	switch($mes2) 
       ...
       case "12": 
		...
		//	$alerta = 'Debe '.$nroHojas2.' hojas de '.$mes.' , tiene que ponerse al dia.';
           ...
           // $nombreCompleto = "No se puede vender al socio por deudas al sindicato"; 		
      //      $nombreCompleto = $nombreSocio['nombre_socio']." ".$nombreSocio['apellido1_socio']." ".$nombreSocio['apellido2_socio']." (Tiene deudas pendientes, no se le puede vender hojas)";
	//	return "<span style=\"position:absolute; background-color:red;\">".$alerta."</span><br /><span>".$nombreCompleto."</span><input type=\"hidden\" name=\"id_movil\" value=\"".$nombreSocio['id_movil']."\">";
		$alerta2 = 'Debe '.$nroHojasfaltantes.' hojas('.$mes2.'),EL SISTEMA NO LE REGISTRARA MAS HOJAS';
       $nombreCompleto = $nombreSocio['nombre_socio']." ".$nombreSocio['apellido1_socio']." ".$nombreSocio['apellido2_socio'];
     	
	return "<span style=\"position:absolute; background-color:red;\">".$alerta2."</span><br /><span>".$nombreCompleto."</span";
	
	//	echo "<script>window.location.href='hoja_control.php'</script>";
		}
	//	$nroHojas = calcHojas($hoy,$nombreSocio['id_movil']);
		//$nroHojas = 17;
		//alerta mes anterior
$mes1 =$mes-1;
...
$dia3 = $maxdays2; 
	...
	switch($mes2) 
      ...
      //      $nombreCompleto = $nombreSocio['nombre_socio']." ".$nombreSocio['apellido1_socio']." ".$nombreSocio['apellido2_socio']." (Tiene deudas pendientes, no se le puede vender hojas)";
	//	return "<span style=\"position:absolute; background-color:red;\">".$alerta."</span><br /><span>".$nombreCompleto."</span><input type=\"hidden\" name=\"id_movil\" value=\"".$nombreSocio['id_movil']."\">";
		$alerta2 = 'Debe prueba'.$nroHojasfaltantes.' hojas('.$mes2.'),EL SISTEMA NO LE REGISTRARA MAS HOJAS';
       $nombreCompleto = $nombreSocio['nombre_socio']." ".$nombreSocio['apellido1_socio']." ".$nombreSocio['apellido2_socio'];
     	
//	return "<span style=\"position:absolute; background-color:red;\">".$alerta2."</span><br /><span>".$nombreCompleto."</span";
	return "<span style=\"position:absolute; background-color:red;\">".$alerta2."</span><br /><span>".$nombreCompleto."</span><input type=\"hidden\" name=\"id_movil\" value=\"".$nombreSocio['id_movil']."\">";
