...
class moroso2
{
	...
	{
		$template = new template;
		$template->SetTemplate('html/lista_moroso.html'); //sets the template for this function
    ...
    //    $resultprestamo = $query->getRows("DISTINCT p.id_prestamo","prestamo_socio p, plan_de_pagos pp","WHERE p.id_prestamo = pp.id_prestamo AND fecha_pago<'".$fecha_actual."'ORDER by p.id_prestamo ");
         $resultprestamo = $query->getRows("DISTINCT p.id_prestamo","prestamo_socio p, plan_de_pagos pp,pertenece pe,movil m","WHERE p.id_prestamo = pp.id_prestamo and pe.id_socio=p.socio and pe.id_movil=m.id_movil and m.id_linea='1' AND fecha_pago<'".$fecha_actual."'ORDER by p.id_prestamo ");
...
$result1 = $query->getRow("count(d.fecha_deposito) as total1","deposito d","WHERE d.fecha_deposito<'".$fecha_actual."' AND d.id_prestamo=".$id_prestamo);
        ...
        $total = $query -> getRow("SUM(monto_deposito) as total","deposito","where id_prestamo = ".$value['id_prestamo']);
			...
			<td>'.$resultdeposito["totaldeposito"].'</td>
					  ...
					  <td><a href="prestamo.php?accion=pago&id='.$id_prestamo.'" title="Pagar Prestamo"><img src="images/pago.gif"></a></td>
	                  </tr></tbody>';
