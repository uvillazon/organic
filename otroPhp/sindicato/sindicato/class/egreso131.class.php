...
class tipoingreso
{
	...
	function registrarTipoIngreso(){
		$template = new template;
		$template->SetTemplate('html/form_egreso131.html');
		$hoy=date("Y-m-d");
    ...
    function modificarTipoIngreso(){
        $query = new query;
        $tipoingreso = $query -> getRow("*","egresos","where id_egreso = ".$_GET[id]);
		$template = new template;
		$template->SetTemplate('html/form_egreso131.html');
    ...
    function deleteTipoIngreso(){
        $query = new query;
        if($query->dbDelete("egresos","WHERE id_egreso = ".$_GET['id']))
            //echo "<script>alert('saldo de ingreso elminado exitosamente')</script>";
        echo "<script>window.location.href='egreso131.php'</script>";
	}
 ...
 $update['monto_dolar'] = $_POST['monto_tipo_ingreso2'];  		
		...
		if($query->dbUpdate($update,"egresos","where id_egreso = ".$_GET['id'])){ //save in the data base
    ...
    function saveTipoIngreso() //save the new Item
    {
		...
		if($query->dbInsert($insert,"egresos")){ //save in the data base
	...
	{
		$template = new template;
		$template->SetTemplate('html/lista_egreso131.html'); //sets the template for this function
		...
		//DataBase Conexion//
		$query = new query;
        $resulthoja = $query->getRows("*","egresos","where fecha = '".$hoy."' ORDER BY id_egreso");
        $init = (($_GET['page'] == "" ? 1 : $_GET['page']) - 1) * 20;
        $resulthoja1 = $query->getRows("*","egresos","where fecha = '".$hoy."' ORDER BY id_egreso LIMIT $init , 20");
            ...
            foreach ($resulthoja1 as $key=>$value) {
				...
				$list .= '<tbody><tr '.$par.'>
				 ...
				 <td>'.$value["concepto"].'</td>
                  ...
                  <td><a href="egreso131.php?accion=modificar&id='.$value["id_egreso"].'" title="Modificar  "><img src="images/edit.gif"></a></td>
                  <td><a href="egreso131.php?accion=delete&id='.$value["id_egreso"].'" onClick="'.$confirm.'" title="Eliminar "><img src="images/delete.gif"></a></td>
                  </tr></tbody>';
            }
            $list.='</table>';
            $list .= paging::navigation(count($resulthoja),"egreso131.php",20);
        } else $list = '<div>No existen registros</div>';
		 $buttonAdd = '<input type="button" value="Nuevo" onclick="window.location.href=\'egreso131.php?accion=registro\'">';
        ...
        $template->SetParameter('add',$buttonAdd);
        $totalDiaBs = $query->getRow("SUM(monto_egreso) as totalDiabs","egresos","where fecha = '".$hoy."'");
        $totalDiaDs = $query->getRow("SUM(monto_dolar) as totalDiadl","egresos","where fecha = '".$hoy."'");
    ...
    function filtroFecha() //list for default the all items
	{
		...
		$query = new query;
        $resulthoja = $query->getRows("*","egresos","where fecha = '".$fechaFiltro."' ORDER BY id_egreso");
        //$init = (($_GET['page'] == "" ? 1 : $_GET['page']) - 1) * 20;
        $resulthoja1 = $query->getRows("*","egresos","where fecha = '".$fechaFiltro."' ORDER BY id_egreso ");
        $numhoja = count($resulthoja1);
	$totalDia = $query->getRow("SUM(monto_egreso) as totalDia","egresos","where fecha = '".$fechaFiltro."'");
      $totalbs = $totalDia['totalDia'];
       $totalDl = $query->getRow("SUM(monto_dolar) as totalD","egresos","where fecha = '".$fechaFiltro."'");
      $totaldolar = $totalDl['totalD'];
      
        $list = '<form name="formRecibir" method="POST" action="egreso131.php?accion=recibir">';
        if($numhoja > 0) {
           $list ='<table border = "1">
              <thead>
