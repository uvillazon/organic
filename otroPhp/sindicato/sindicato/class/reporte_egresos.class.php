...
class reporte_movil
{
    ...
    function DiasReporte() //list for default the all items
	{
		...
		$query = new query;
       	 $resultmovil1 = $query->getRows("*","egreso_lubricante","WHERE tipo='0' AND fecha >= '".$fechaInicio."' and fecha <= '".$fechaFin."' ORDER BY fecha DESC");
		        $hoy = date("Y-m-d");
        $numsocio = count($resultmovil1);
         $totalDiaBs = $query->getRow("SUM(monto_egreso) as totalDiabs","egreso_lubricante","where tipo='0' AND fecha >= '".$fechaInicio."' and fecha <= '".$fechaFin."'");
	...
	{
		$template = new template;
		$template->SetTemplate('html/reporte_egresos.html'); //sets the template for this function
