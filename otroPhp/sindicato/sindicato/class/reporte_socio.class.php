...
class reporte_socio
{
	...
	$resultmovil1 = $query->getRows("*","movil","where id_linea='2' ");
	...
	$diaschofer = count($hojaschofer);
		...
		//$hojascontrol = $query->getRows("*","hoja_control h, movil m","where h.id_movil=m.id_movil and m.id_linea='2'");
		//$ahorroSocio = $query->getRow("SUM(tt.monto_transaccion) as total","hoja_control h, movil m, tipo_transaccion tt, tener t, linea l","where h.id_movil = m.id_movil and m.id_linea = l.id_linea and t.id_linea = l.id_linea and tt.id_tipo_transaccion = t.id_tipo_transaccion and t.estado = 'Activo' and tt.id_tipo_transaccion = '2' and h.id_movil = ".$value['id_movil']." and h.alquiler='0' and h.fecha_falta >= '".$fechaInicio."' and h.fecha_falta <= '".$fechaFin."'");
        //         $ahorroSo= $ahorroSocio['total'];
	   //$ahorroChofer = $query->getRow("SUM(tt.monto_transaccion) as total","hoja_control h, movil m, tipo_transaccion tt, tener t, linea l","where h.id_movil = m.id_movil and m.id_linea = l.id_linea and t.id_linea = l.id_linea and tt.id_tipo_transaccion = t.id_tipo_transaccion and t.estado = 'Activo' and tt.id_tipo_transaccion = '3' and h.id_movil = ".$value['id_movil']." and h.alquiler='0' and h.id_chofer='0' and h.fecha_falta >= '".$fechaInicio."' and h.fecha_falta <= '".$fechaFin."'");
        //      $ahorroCho= $ahorroChofer['total'];
        	  ...
        	  $ahorroCho= $ahorroChofer['total'];
			    $totalahorro2=$ahorroCho + $ahorroSo;
			   
//modificado desde aqui
			  //$totalahorro=$ahorroCho + $ahorroSo;
		//$totalahorro2=$ahorroCho + $ahorroSo;
		$ahorroini = $query->getRow("ahorro_inicial","movil","where id_movil = ".$value['id_movil']." and fecha_ahorro_inicial >= '".$fechaInicio."' and fecha_ahorro_inicial <= '".$fechaFin."'");
        $ahorroinicial= $ahorroini['ahorro_inicial'];
		
		$totalahorro=$totalahorro2 + $ahorroinicial;
//modificacion con ahorro inicial
	   ...
	   $ahorropagado = $query->getRow("SUM(a.monto_pago_ahorro) as total","pago_ahorro a, movil m","where  a.id_movil=m.id_movil and a.id_movil = ".$value['id_movil']." and a.id_chofer='0' and a.id_alquiler='0' and a.fecha_ahorro_dia >= '".$fechaInicio."' and a.fecha_ahorro_dia <= '".$fechaFin."'");
		...
		$list .= '<tbody><tr '.$par.'>
                 ...
                 <td>'.$ahorroini["ahorro_inicial"].'</td>
