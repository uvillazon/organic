    ...
    function DiasReporte() //list for default the all items
	{
        ...
        */
		...
		$id_chofer = $value['id_chofer'];
				...
				$hojas = $query->getRows("*","hoja_control h","where id_chofer = ".$value['id_chofer']."  and fecha_falta >= '".$fechaInicio."' and fecha_falta <= '".$fechaFin."'");
				$numHojas = count($hojas);
                ...
                $list .= '<tbody><tr '.$par.'>
                 ...
                 <td>'.$ahorrocancelado.'</td>
				  <td>'.$saldoahorro.'</td>
	                         ...
	                         <td><a href="print_reporte_ahorrochofer1.php?accion=imprimir&id='.$value["id_chofer"].'&fi='.$_GET["fechaInicio"].'&ff='.$_GET["fechaFin"].'" 
