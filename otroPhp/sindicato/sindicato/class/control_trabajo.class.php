...
class dias_trabajados
{
    ...
    function DiasTrabajados() //list for default the all items
	{
        ...
        if($numsocio > 0) {
            ...
            foreach ($resultsocio as $key=>$value) {
               ...
               // $hojas = $query->getRows("*","hoja_control","where id_movil = ".$value['id_movil']." and (fecha_de_compra >= '".$fechaInicio."' and fecha_de_compra <= '".$fechaFin."') or (fecha_falta >= '".$fechaInicio."' and fecha_falta <= '".$fechaFin."')");
	...
	{
		$template = new template;
		$template->SetTemplate('html/lista_control_trabajo.html'); //sets the template for this function
