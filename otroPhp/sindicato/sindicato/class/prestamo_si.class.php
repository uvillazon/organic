...
class prestamo
{
	...
	function registrarPrestamo(){
		$query = new query;
		$template = new template;
		$template->SetTemplate('html/form_prestamo_si.html');
        $template->SetParameter('licencia',"<input name=\"licencia\" type=\"text\" id=\"licencia\" onblur=\"if(document.mframe.licencia.value > 0) {ajax('nombreSocio','prestamo_si.php?accion=buscarSocio&licencia=' + document.mframe.licencia.value, '');}\" size=\"8\">");
		
		$template->SetParameter('licencia2',"<input name=\"licencia2\" type=\"text\" id=\"licencia2\" onblur=\"if(document.mframe.licencia2.value > 0) {ajax('nombreChofer','prestamo_si.php?accion=buscarChofer&licencia=' + document.mframe.licencia2.value, '');}\" size=\"8\">");
		
		$tipoprestamolist = $query->getRows("*","tipo_prestamo","where interes='0.00'");
    ...
    function modificarPrestamo(){
		...
		$template = new template;
		$template->SetTemplate('html/form_prestamo_si.html');
        $template->SetParameter('licencia',"<input name=\"licencia\" type=\"text\" id=\"licencia\" onblur=\"if(document.mframe.licencia.value > 0) {ajax('nombreSocio','prestamo_si.php?accion=buscarSocio&licencia=' + document.mframe.licencia.value, '');}\" size=\"8\" value=".$socio['num_licencia'].">");
		
		$template->SetParameter('licencia2',"<input name=\"licencia2\" type=\"text\" id=\"licencia2\" onblur=\"if(document.mframe.licencia2.value > 0) {ajax('nombreChofer','prestamo_si.php?accion=buscarChofer&licencia=' + document.mframe.licencia2.value, '');}\" size=\"8\" value=".$chofer['num_chofer'].">");
    ...
    function deletePrestamo(){
		...
		if(count($deposito) != 0) {
		 $query->dbDelete("deposito","WHERE id_prestamo = ".$_GET['id']);
          ...
          echo "<script>window.location.href='prestamo_si.php'</script>";
	       }
	    }
		}
    ...
    function savePrestamo() //save the new Item
    {
        ...
        $insert['fecha_prestamo'] = date("Y-m-d");
	...
	function calcularPlan()
	{
		...
		echo "<script>window.location.href='prestamo_si.php?accion=plan&id=".$_POST['id_prestamo']."'</script>";
	}
	...
	function guardarPago()
	{
		...
		//}
		echo "<script>window.location.href='prestamo_si.php?accion=pago&id=".$_POST['id_prestamo']."'</script>";
	}
	
	function planPagos()
	{
		$template = new template;
		$template->SetTemplate('html/lista_pagos_si.html'); //sets the template for this function
        ...
        $buttonAdd = '<input type="button" value="CALCULAR Y GUARDAR" onclick="window.location.href=\'prestamo_si.php?accion=calcularPlan&id='.$_GET['id'].'\'">';
	...
	{
		$template = new template;
		$template->SetTemplate('html/lista_pagoshechos_si.html'); //sets the template for this function
		...
		$template->SetParameter('interes',$interes['interes']);
		$template->SetParameter('total',$totalPago);
        $buttonAdd = '<input type="button" value="GUARDAR" onclick="window.location.href=\'prestamo_si.php?accion=guardarPago&id='.$_GET['id'].'\'">';
	...
	{
		$template = new template;
		$template->SetTemplate('html/lista_prestamo_si.html'); //sets the template for this function
        ...
        $resultprestamo = $query->getRows("*","prestamo_socio a, tipo_prestamo b","WHERE b.interes='0.00' and a.id_tipo_prestamo=b.id_tipo_prestamo ");
 ...
 $resultprestamo1 = $query->getRows("*","prestamo_socio  a, tipo_prestamo b","WHERE b.interes='0.00' and a.id_tipo_prestamo=b.id_tipo_prestamo ORDER by id_prestamo desc");
	   ...
	   //$resultprestamo1 = $query->getRows("*","prestamo_socio  a, tipo_prestamo b","WHERE a.id_tipo_prestamo=b.id_tipo_prestamo ");
        ...
        if($numprestamo > 0) {
            ...
            foreach ($resultprestamo1 as $key=>$value) {
                ...
                $list .= '<tbody><tr '.$par.'>
				 <td>'.$value["id_prestamo"].'</td>
				  ...
				  <td><a href="prestamo_si.php?accion=plan&id='.$value["id_prestamo"].'" title="Plan de pagos"><img src="images/plan.gif"></a></td>
				  <td><a href="prestamo_si.php?accion=pago&id='.$value["id_prestamo"].'" title="Pagar Prestamo"><img src="images/pago.gif"></a></td>
                  <td><a href="prestamo_si.php?accion=modificar&id='.$value["id_prestamo"].'" title="Modificar Prestamo"><img src="images/edit.gif"></a></td>
                  <td><a href="prestamo_si.php?accion=delete&id='.$value["id_prestamo"].'" onClick="'.$confirm.'" title="Eliminar prestamo"><img src="images/delete.gif"></a></td>
            ...
            $list.='</table>';
            //$list .= paging::navigation(count($resultprestamo),"prestamo_si.php",10);
        } else $list = '<div>No existen prestamos registrados</div>';
        $buttonAdd = '<input type="button" value="Nuevo" onclick="window.location.href=\'prestamo_si.php?accion=registro\'">';
        $template->SetParameter('add',$buttonAdd);
