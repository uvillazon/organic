...
function Display(){
       ...
       // $resultsocio= $query->getRows("*","pertenece p","order by p.id_movil");
        ...
        /*$resultmovil1 = $query->getRows("*","socio m,pertenece p, hoja_control h","where m.id_socio=h.id_socio and p.id_socio=h.id_socio and h.fecha_de_compra >= '".$fechaInicio."' and h.fecha_de_compra <= '".$fechaFin."'");
        */
        $hoy = date("Y-m-d");
      ...
      <th>Pagado</th>
		...
		//$totalahorro2=$ahorrocomochofer + $ahorroSocio;

		$ahorroSocio = $query->getRow("SUM(h.achofer) as total","hoja_control h, movil m, linea l","where h.id_movil = m.id_movil and m.id_linea = l.id_linea and h.id_movil = ".$value['id_movil']." and h.alquiler='0' and fecha_de_compra >= '".$fechaInicio."' and fecha_de_compra <= '".$fechaFin."'");
              $ahorroSocio= $ahorroSocio['total'];
              $ahorroChofer = $query->getRow("SUM(h.asocio) as total","hoja_control h, movil m, linea l","where h.id_movil = m.id_movil and m.id_linea = l.id_linea and h.id_movil = ".$value['id_movil']." and h.alquiler='0' and fecha_de_compra >= '".$fechaInicio."' and fecha_de_compra <= '".$fechaFin."'");
...
//$totalahorro=$totalahorro + $ahorroinicial;
